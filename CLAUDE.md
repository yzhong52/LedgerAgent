...
- **No optional parameters when possible.** Prefer required parameters over `foo?: T`. Optional parameters force the function to silently handle `undefined` internally, hiding decisions that the caller should make explicitly. Use optional only when absence is genuinely meaningful (e.g. a callback that may or may not be provided).
- **No default parameter values when possible.** Defaults hide intent and make call sites ambiguous — `foo(a, undefined, c)` is a sign the default is load-bearing but invisible. If a default is needed, place it at the single public entry point (e.g. the CLI flag or the top-level exported function) rather than deep in the implementation. Exception: CLI option defaults are fine since they are part of the user-facing UX.
